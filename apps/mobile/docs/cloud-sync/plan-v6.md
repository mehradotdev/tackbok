...
- **Connected-account label.** `drive.appdata` alone conveys no identity. The account label shown in §11 comes from the sign-in layer's basic profile (`openid email` — the default identity scopes of Google sign-in) and displays the full email so the chosen Google account is unambiguous. It never enters SQLite, the vault, logs, diagnostics, or analytics. Both platforms keep the selected email in a separate SecureStore item beside the credentials so the label survives process death without another user-info request; Android also passes it to Play services so silent authorization cannot drift to another device account. Disconnect deletes the email with the credentials. This handling is disclosed in §10, and the reserved v1 `cloud_vault.account_label` column remains null. If the chosen iOS path cannot supply a label without extra scopes, request `email` explicitly there too; if the user-agent flow makes that awkward, the label falls back to "Google Drive" — the label is cosmetic and must never block connection.
...
- Scopes: `drive.appdata` for data, plus the sign-in layer's basic identity (`openid email`) used to display the connected account's full email. The email is excluded from SQLite, the vault, logs, analytics, and diagnostics exports; both platforms keep it only in SecureStore beside the credentials and remove it on Disconnect. Android additionally uses it for account pinning. Both authorization paths verify that granular consent actually granted `drive.appdata`; a partial identity-only grant is rejected with an actionable retry message. The disclosure screen states this boundary and tells the user to select the Drive checkbox when Google shows it.
- **Encryption: none in v1** (rationale in §1). Disclosure text: _"Backups are encrypted in transit and at rest by Google Drive, but are not end-to-end encrypted."_ Privacy policy (plain language, per site conventions) updated in the same release.
...
**Phase 0 — format ADR, fixtures, and four blocking spikes.** ✅ **Completed — gate conditionally closed 2026-08-09** (owner waiver: no physical devices available; evidence, re-scoped items, and the Phase-3 merge-blocking obligations are recorded in [phase0/gate.md](phase0/gate.md)).
...
**Phase 1 — normalized model + transactional outbox.** ✅ **Re-reviewed and re-closed 2026-08-09 after owner-review remediation** ([gate evidence](phase1/gate.md)). Schema-only migration + checkpointed backfill (§4.2, including the tag/prompt unique-constraint relaxation and the **profile migration out of Zustand**), transaction-scoped repositories, all write paths routed (including profile and edit-time media removal through the ledger, §7.1), generation counters + per-device edit sequence, tombstone/hard-delete split with the retained-media ledger, ZIP export/import with additive stable-ID fields on the normalized model (§3). **Repositories/dual-write go live before the backfill starts, and the reconciliation pass runs before migration is declared complete (§4.2).** Gate: killing the app after any domain commit — or mid-backfill — never loses queued sync intent, retained media, or ID-assignment consistency; the concurrent-write-during-backfill matrix (edit, delete, tag membership, asset replacement, profile change, restart) leaves normalized tables, legacy fields, and the outbox consistent.

**Phase 2 — engine against the fake provider.** ✅ **Re-reviewed and re-closed 2026-08-09 after owner-review remediation** ([gate evidence](phase2/gate.md)). Canonical codec, hashing, `ancestry/` (head sets, merge-base sets, orphan staging, validation caps), provisional-version construction, entity-version construction including system versions with `recoveries`, outbox coalescing + generation settle rule + **Apply CAS + provisional-chain rebase**, state machine, N-head conflict package with recovered-asset remapping and scalar alternates, multi-entity apply ordering + tombstoned-tag recovery, **checkpointed initial seeding**, **revocation handling against the fake provider (per-device observed-marker behavior, purge resume, both kinds)**, crash recovery, provider contract suite (at-least-once duplicates, out-of-order delivery, **delete idempotency**). Gate: the exact deterministic schedules recorded in the 34-scenario catalog, the general-chaos seeds, and the separate revocation schedules pass; durable restart from the production SQLite engine remains a Phase-4 integration obligation rather than a claim of this in-memory gate.

**Phase 3 — Google Drive adapter.** ✅ **Conditionally closed 2026-08-09** ([gate evidence](phase3/gate.md), [physical-device waiver](phase3/waiver.md)). Owner probe passes ran against real Google Drive on both platforms with disposable accounts; 5 of 7 merge-blocking probes closed on evidence, the other 2 dispositioned by the waiver (natural session expiry waived on trigger; release-signed builds deferred to block store submission, not Phase 4). Two Android authorization findings are recorded in [phase3/findings/](phase3/findings/): 0002 (Disconnect did not disconnect) is fixed and re-verified on-device; 0001 (post-revocation dead window) is open and not merge-blocking. Authorization per spike-d ADR (per §3.3–3.4) + SecureStore handling, vault-namespace key mapping via appProperties, change cursors, resumable blob upload/download with session expiry handling, duplicate tolerance, **permanent-delete operations + revocation purge against real Drive**, error normalization. Gate: contract suite passes against real Drive; physical-device ~200 MB media fixture passes; **a real end-to-end connect → grant → refresh-after-expiry → revoke cycle passes on physical Android and iOS devices**.

**Phase 4 — runtime + UI.** `SyncRuntime` startup in `_layout.tsx`, AppState/network triggers, debounce, background-task registration, all screens/sheets from §11 (including the onboarding restore entry point + tagline update, §11.5, and the revocation notices), translations, accessibility. **Merge-blocking readiness obligation:** sync remains disabled until `isNormalizedModelReady()` returns true, and a failed/resumable backfill gets an in-session retry path (not only retry-on-next-launch). Gate: setup never requests notification permission; an offline edit immediately shows as safely queued; onboarding completes fully offline with the Drive row failing gracefully.
