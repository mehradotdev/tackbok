...
- **Connected-account label.** `drive.appdata` alone conveys no identity. The account label shown in §11 comes from the sign-in layer's basic profile (`openid email` — the default identity scopes of Google sign-in), is immediately masked and kept only in memory, never enters SQLite, the vault, logs, diagnostics, or analytics, and is disclosed in §10. The reserved v1 `cloud_vault.account_label` column remains null. If the chosen iOS path cannot supply a label without extra scopes, request `email` explicitly there too; if the user-agent flow makes that awkward, the label falls back to "Google Drive" — the label is cosmetic and must never block connection.
...
- Scopes: `drive.appdata` for data, plus the sign-in layer's basic identity (`openid email`) used **only** to derive a masked, in-memory connected-account label; neither raw nor masked identity is persisted or included in the vault, logs, analytics, or diagnostics exports. The disclosure screen states both.
