...
export const zhCN: Translations = {
  ...
  'Cloud data is protected in transit and by your storage provider. Tackbok does not end-to-end encrypt cloud backups in this version.': '云数据在传输过程中及由存储提供商进行保护。此版本的 Tackbok 不对云备份进行端到端加密。',
  ...
  'Google shares basic account identity so Tackbok can show a masked account label. Your email is never added to backups, logs, diagnostics, or analytics.': 'Google 会提供基本账户信息，以便 Tackbok 显示隐藏部分内容的账户标签。你的电子邮件绝不会加入备份、日志、诊断或分析数据。',
  ...
  'This connects storage only. It does not create a Tackbok account.': '这只会连接存储空间，不会创建 Tackbok 账户。',
