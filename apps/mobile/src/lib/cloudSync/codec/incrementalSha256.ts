...
/** Incremental SHA-256 for bounded network verification. */
export class IncrementalSha256 {
  ...
  digestHex(): string {
    ...
    view.setUint32(final.length - 8, Math.floor(bitLength / 0x100000000), false);
    view.setUint32(final.length - 4, bitLength >>> 0, false);
  ...
  private process(block: Uint8Array): void {
    ...
    for (let index = 0; index < 64; index++) {
      ...
      const t1 = (h + s1 + choice + ROUND_CONSTANTS[index] + words[index]) >>> 0;
