...
export class MediaPartialFileSink implements MediaDownloadSink {
  ...
  async appendAndSync(bytes: Uint8Array): Promise<void> {
    ...
    try {
      ...
      throw new LocalStorageError('local-storage-full', 'media-partial-append-failed');
    }
  }
  ...
  async reset(): Promise<void> {
    try {
      ...
      throw new LocalStorageError('local-storage-full', 'media-partial-reset-failed');
    }
  }
  ...
  async verifyAndPromote(expectedByteLength: number, expectedSha256: string): Promise<void> {
    ...
    try {
      ...
      throw new LocalStorageError('local-storage-full', 'media-atomic-promotion-failed');
    }
  }
}
