...
- **Default Scope:** 85% of the work is focused on the React-Native Expo project. Unless explicitly stated otherwise by the user, assume all tasks, commands, and file modifications should be confined to the `./apps/mobile` directory. Do not modify files outside of `apps/mobile` unless required.
...
- **Type Safety:** Avoid `as any` or `any` at all costs. Try to infer types from functions as much as possible or write strict, robust types/interfaces.
